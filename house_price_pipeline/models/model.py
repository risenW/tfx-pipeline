"""TFX House Pricing Model.

A DNN keras model which uses features defined in features.py
"""
...
def _build_keras_model(hidden_units, learning_rate):
    """Creates a DNN Keras model for predictiing house price.
  ...
  """
    ...
    categorical_columns = [
        tf.feature_column.categorical_column_with_identity(
            key, num_buckets=features.VOCAB_SIZE + features.OOV_SIZE, default_value=0
        )
...
def run_fn(fn_args):
  ...
  """
    ...
    train_dataset = _input_fn(
        fn_args.train_files, tf_transform_output, TRAIN_BATCH_SIZE
    )
